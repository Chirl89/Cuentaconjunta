...
- **Paso 3 (v0.3)**: **Esquema de Base de Datos Supabase (PostgreSQL)**
  - Migraciones SQL: tablas `users`, `bank_connections`, `accounts`, `transactions`, `rules`, `categories` y `settlements` (con soporte para estados: `pending_assignment`, `auto_assigned`, `verified`), tipos TypeScript y cliente de conexión.
...
- **Paso 7 (v0.7)**: **Motor de Reglas Configurables, Auto-Asignación y Reasignación**
  - Lógica de reglas personalizadas por comercio/cuenta, auto-asignación inteligente, reversibilidad/reasignación de cualquier gasto y soporte de splits 50/50 o personalizados.
...
- **Paso 9 (v0.9)**: **Dashboard Principal: Inbox de Asignación Rápida, Validación de Reglas y Feed UI**
  - Interfaz de usuario PWA: Backlog de triage rápido al abrir la app (asignar a Persona A, Persona B o Ambos con 1 toque), pestaña de validación de auto-asignados, tarjeta de balance en vivo y barras de presupuesto.

- **Paso 10 (v0.10)**: **Gestión de Bancos, Tarjetas y Monitor de Consentimiento PSD2**
  - Panel de administración de cuentas/tarjetas, editor de reglas de usuario, indicador de días restantes del permiso bancario y botón de re-autorización biométrica.
